...
- **Floor and ceiling** (`minBuyAmount` / `maxBuyAmount`) — the two signed buy-amount fields in a proposal. `minBuyAmount` is the floor: the hard revert threshold the Trampoline's delta check enforces on-chain. `maxBuyAmount` is the ceiling: the clearing-price commitment used for scoring, gas-cut sizing, and settlement encoding. When both are equal, the proposal behaves as a fixed-amount commitment. When `minBuyAmount` is lower (sell orders only), the sub-solver opts into aggressive slippage — the gap between `maxBuyAmount` and the actual delivery is charged against the sub-solver's escrow. Buy orders hard-reject `minBuyAmount != maxBuyAmount`. See [the design document](design-document#order-flow).
